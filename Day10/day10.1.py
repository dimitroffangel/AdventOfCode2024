...
def check_adjacent_position(adjacent_position, current_vertex, initial_position):
    ...
    if map[current_vertex.y][current_vertex.x] + 1 == map[adjacent_position.y][adjacent_position.x] and not used_map[adjacent_position.y][adjacent_position.x].is_used:
        used_map[adjacent_position.y][adjacent_position.x].is_used = True
...
for starting_position in starting_positions:
    ...
    for row, map_row in enumerate(used_map):
        for col, vertex in enumerate(map_row):
            vertex.is_used = False
            used_map[row][col].number_of_paths = 0
