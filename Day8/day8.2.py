...
for type_of_satellitie in types_of_satellites:
    ...
    for position_index, position in enumerate(types_of_satellites[type_of_satellitie]):
        ...
        for other_position in types_of_satellites[type_of_satellitie][position_index + 1:]:
            ...
            left_antipode = position
            while left_antipode.x >= 0 and left_antipode.x < COL_SIZE and left_antipode.y >= 0 and left_antipode.y < ROW_SIZE:
                if not added_antinodes.__contains__(left_antipode):
                ...
                left_antipode = Position(left_antipode.x + x_difference * lhs_x_offset, left_antipode.y + y_difference * lhs_y_offset)

            right_antipode = other_position
            while right_antipode.x >= 0 and right_antipode.x < COL_SIZE and right_antipode.y >= 0 and right_antipode.y < ROW_SIZE:
                if not added_antinodes.__contains__(right_antipode):
                ...
                right_antipode = Position(right_antipode.x + x_difference * rhs_x_offset, right_antipode.y + y_difference * rhs_y_offset)
