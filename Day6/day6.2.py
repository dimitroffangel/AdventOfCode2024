...
for current_y, current_y_line in enumerate(guard_map):
    for current_x, current_symbol in enumerate(current_y_line):
        if current_symbol != OBSTACLE_SYMBOL and current_symbol != FREE_SYMBOL and current_symbol != '\n':
            guard_position = Position(current_x, current_y)
            has_guard_position_reached = True
            current_guard_symbol = current_symbol
            break
    ...
    for y, line in enumerate(guard_map):
        ...
        for x, symbol in enumerate(line):
            ...
            for adjacent_x in range(guard_map_line_size)[1:]:
                if x - adjacent_x >= 0 and guard_map[y][x - adjacent_x] == OBSTACLE_SYMBOL and x - adjacent_x + 1 > nearestObstacles.lhs_position.x:
                    nearestObstacles.lhs_position = Position(x - adjacent_x + 1, y)

                if x + adjacent_x < guard_map_line_size and guard_map[y][x + adjacent_x] == OBSTACLE_SYMBOL and x + adjacent_x - 1 < nearestObstacles.rhs_position.x:
                    nearestObstacles.rhs_position = Position(x + adjacent_x - 1, y)

            for adjacent_y in range(guard_map_size)[1:]:
                if y - adjacent_y >= 0 and guard_map[y - adjacent_y][x] == OBSTACLE_SYMBOL and y - adjacent_y + 1 > nearestObstacles.up_position.y:
                    nearestObstacles.up_position = Position(x, y - adjacent_y + 1)

                if y + adjacent_y < guard_map_size and guard_map[y + adjacent_y][x] == OBSTACLE_SYMBOL and y + adjacent_y - 1 < nearestObstacles.down_position.y:
...
while not has_guard_reached_border:
    ...
    if ((next_guard_position.y == 0 and current_guard_symbol == '^') or \
        ...
        (next_guard_position.x >= guard_map_line_size - 1 and current_guard_symbol == '>')) and \
            ...
            if used_positions[next_guard_position.y][next_guard_position.x].current_symbol == FREE_SYMBOL:
    ...
    elif used_positions[next_guard_position.y][next_guard_position.x].current_symbol == FREE_SYMBOL:
        used_positions[next_guard_position.y][next_guard_position.x].previous_position = guard_position
        used_positions[next_guard_position.y][next_guard_position.x].current_symbol = current_guard_symbol
...
def try_creating_cycle(cycle_position, guard_position, current_guard_symbol):
    ...
    used_map_directions = [ [0 for _ in line ] for line in guard_map]
    ...
    while not has_guard_position_reached and not has_entered_cycle:
        ...
        if used_map_directions[guard_position.y][guard_position.x] > 4:
            has_entered_cycle = True

        used_map_directions[guard_position.y][guard_position.x] = used_map_directions[guard_position.y][guard_position.x] + 1
    ...
    for current_x, has_guard_been_there_initially in enumerate(line):
        ...
        if has_guard_been_there_initially.current_symbol != OBSTACLE_SYMBOL and has_guard_been_there_initially.current_symbol != FREE_SYMBOL and \
            ...
            for obstacle in obstacle_positions:
                ...
                current_guard_position = used_positions[current_y][current_x].previous_position
                ...
                number_of_new_paths = number_of_new_paths + 1
                ...
                for horizontal in range(guard_map_line_size):
                    ...
                    cached_closest_obstacle_distance[current_y][horizontal].up_position = changed_horizontal_line[horizontal].up_position
                    cached_closest_obstacle_distance[current_y][horizontal].down_position = changed_horizontal_line[horizontal].down_position
                ...
                for vertical in range(guard_map_size):
                    ...
                    cached_closest_obstacle_distance[vertical][current_x].lhs_position = changed_vertical_line[vertical].lhs_position
                    cached_closest_obstacle_distance[vertical][current_x].rhs_position = changed_vertical_line[vertical].rhs_position

                if has_found_cycle_result:
                    print(f"Found cycle at {current_x}, {current_y}")
                break
